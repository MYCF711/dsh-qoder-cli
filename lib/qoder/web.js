...
export async function listAccounts(deps) {
  // `deps` is the FLAT object `registerQoderRoutes` receives from lib/index.js —
  // the same shape every sibling handler uses (`deps.store`, `deps.models`, …).
  //
  // It is deliberately NOT unwrapped a second time. An earlier revision read
  // `dprism.deps` / `dprism.discover` / `dprism.dpapiUnprotect`, but the only
  // caller passes the flat object, so `deps` was `undefined` and the optional
  // chain below threw on the OBJECT before it could guard the property:
  //   TypeError: Cannot read properties of undefined (reading 'listAlternateCredentials')
  // That surfaced in the UI as "无法读取账号列表", while every other panel still
  // reported the account as healthy.
  //
  // `discover` / `dpapiUnprotect` / `listAlternateCredentials` are OPTIONAL
  // here: lib/index.js does not supply them, so an absent value must mean
  // "no alternates to list", never a crash.
...
export async function activateAccount(deps, uid) {
  // See `listAccounts` for why these are read flat and treated as optional:
  // lib/index.js supplies neither `discover` nor `dpapiUnprotect`, so an absent
  // value must mean "no alternates", not a TypeError.
  const discover = deps.discover
  const dpapiUnprotect = deps.dpapiUnprotect
  ...
  // Collect alternate credentials. `listAlternateCredentials` needs BOTH
  // discover and dpapiUnprotect to read other credential stores; when either is
  // absent (the shape lib/index.js passes) there is nothing to walk, so skip it
  // instead of throwing from inside the generator call.
  if (discover !== undefined && dpapiUnprotect !== undefined) {
...
export async function probeSpecificAccount(deps, uid) {
  // Same flat/optional contract as `listAccounts`. Without both helpers the
  // alternate-credential walk is impossible, so the uid simply is not found.
  const discover = deps.discover
  const dpapiUnprotect = deps.dpapiUnprotect
