...
/**
 * Decide whether a peeked SSE body carries an in-band error on a 200 stream.
 *
 * Qoder's gateway reports failures **inside a 200 response**, so the status code
 * alone proves nothing (see docs/EXPERIENCE-HANDOFF.md §4). Measured shapes:
 *
 *   1. `event: error` + `data: {...}`          — Qoder's own SSE error event
 *   2. `data: {"type":"invalid_model_error"}`  — Qoder's typed error frame
 *   3. `data: {"code":"invalid_model_error"}`  — bare code frame
 *   4. `data: {"error":{"code":…,"message":…}}` — OpenAI-standard error envelope
 *   5. `data: {"type":"error","error":{…}}`    — OpenAI-standard typed variant
 *
 * Verifier finding V2-F1: the original check matched only shapes 1 and 2, so
 * shapes 3–5 were delivered to the caller as a **successful empty answer** —
 * exactly the "200 means success" failure this module exists to prevent. Shape
 * 4 is the most likely real-world形态 because it is the OpenAI envelope, which
 * is what OpenAI-compatible clients (and pi-ai) emit and expect.
 *
 * A frame only counts when it parses as JSON **and** carries an error signal;
 * a plain text delta that merely mentions "error" must NOT trip this.
 */
function frameCarriesError(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    // Shape 4/5: an `error` object at the top level, or a typed error frame.
    if (parsed.error !== undefined && parsed.error !== null) return true
    // Shape 2/3: a Qoder/OpenAI error code or type.
...
/** Extract the human message from an in-band error frame, covering all shapes. */
function errorMessageFromFrame(text) {
  for (const line of text.split(/\r?\n/)) {
    ...
    try {
      ...
      // OpenAI envelope: {"error":{"message":…}}; Qoder frame: {"message":…}
