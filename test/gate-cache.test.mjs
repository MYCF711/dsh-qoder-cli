...
if (mod.__err) {
  ...
  if (helper.__err || libWasm.__err) {
    ...
    // Case (1) — the regression itself, called with no arguments at all.
    await test('init(): calling with no argument succeeds (was ERR_INVALID_ARG_TYPE)', async () => {
      await assert.doesNotReject(
        () => initLib(),
        'lib wasm init() must fall back to its bundled WASM_PATH',
      )
      const exports = await initLib()
      assert.equal(typeof exports, 'object', 'init() must resolve the wasm instance exports')
      assert.ok(exports !== null, 'init() must not resolve null')
    })

    await test('init(): the no-argument path loads the same export surface as an explicit path', async () => {
      const viaDefault = await initLib()
      const viaExplicit = await initLib(WASM_PATH)
      assert.deepEqual(
        Object.keys(viaDefault).sort(),
        Object.keys(viaExplicit).sort(),
        'default and explicit paths must load the same export surface',
      )
      assert.ok(
        Object.keys(viaDefault).length > 0,
        'the wasm instance must actually export functions',
      )
    })
    ...
    await test('resolveProbeToken: reads a token from a temp .auth store (configHome)', async () => {
      ...
      try {
        // Helper instance first: it must be initialised BEFORE the encryption
        // below. The library instance is initialised by resolveProbeToken's own
        // bare init(), which does not touch this module's instance.
        await initHelper(WASM_PATH)
        ...
        await writeFile(
          join(home, '.auth', 'user'),
          credentialStorageEncrypt(JSON.stringify(credential), MACHINE_ID.slice(0, 16)),
          'utf8',
        )
