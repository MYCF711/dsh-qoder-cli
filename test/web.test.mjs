...
if (web) {
  ...
  // ------------------------------
  await testAsync('registerQoderRoutes: mock webServer collects all nine handlers at their exact paths', async () => {
    const { deps } = mockDeps()
    const registered = collectRoutes(deps)
    assert.equal(registered.length, 9, 'all nine routes must register')
    ...
    assert.ok(byPath.has('/plugins/dsh-qoder-cli/accounts'), 'accounts list path must register')
    assert.ok(byPath.has('/plugins/dsh-qoder-cli/accounts/activate'), 'accounts activate path must register')
    assert.ok(byPath.has('/plugins/dsh-qoder-cli/accounts/probe'), 'accounts probe path must register')
  ...
  await testAsync('registerQoderRoutes: disposing the effect removes every registered handler', async () => {
    ...
    assert.equal(registeredCount, 9, 'setup must register all nine routes')
    returnedDisposer()
  ...
  // ------------------------------
  // TEST GROUP 6: probeSpecificAccount resolves the CURRENT account's uid
  // ------------------------------
  //
  // Defect report F-1 (from the t12 UI task) claimed probing the CURRENT uid always
  // 404s because `probeSpecificAccount` only walks `listAlternateCredentials()`.
  //
  // Captain re-measurement: NOT reproduced. With the real deps, discovery yields the
  // CLI-store credential whose `uid` equals the current account, so the uid match at
  // web.js:544 succeeds and probe answers { available: true }. F-1 does not hold on
  // this machine, so no implementation change is warranted.
  //
  // What IS worth pinning is the NEGATIVE side: an unknown uid must still fail rather
  // than degrade into "any uid works". That is the assertion kept below.
  await testAsync('probeSpecificAccount: an unknown uid is rejected (does not degrade to "always available")', async () => {
    const { deps } = mockDeps()
    const probeDeps = {
      ...deps,
      store: { resolve: async () => ({ credential: { token: 'current-token' }, machineId: 'mid-current' }) },
      listAlternateCredentials: async function* () {},
      discover: null,
      dpapiUnprotect: null,
      probeCredential: async () => true,
    }

    await assert.rejects(
      () => web.probeSpecificAccount(probeDeps, 'nonexistent-uid'),
      /not found/i,
