...
/**
 * Models enabled in the pickers when the user has chosen none.
 *
 * `qfmodel` (Qwen3.8-Flash) is the deliberate choice, for three measured
 * reasons:
 *
 *  1. It WORKS. Measured on the real account 2026-09-21, same token and same
 *     moment: `qfmodel` answered normally while `qmodel`, `gmodel`, `kmodel` and
 *     `dmodel` all answered "You've reached your credit usage limit".
 *  2. It is free. The catalog snapshot records `price_factor: 0`, `is_free:
 *     true`. The other keys are billable, and the gateway's credit gate refuses
 *     them for an account whose plan quota (`userQuota.total`) is 0 even when an
 *     add-on balance is untouched.
 *  3. It is a reasoning model, so it is not a capability concession.
 *
 * This list used to be effectively `qmodel`, which is the one key the gateway
 * now refuses — a working free model sat in the catalog unused behind a key that
 * could not answer. Keeping the default here (rather than inline in index.js)
 * makes the choice assertable by the catalog tests.
 */
export const DEFAULT_ENABLED_MODEL_IDS = Object.freeze([
  'qmodel_38max',
  'qfmodel',
])
