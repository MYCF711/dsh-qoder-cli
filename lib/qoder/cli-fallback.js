 ...
 *
 * `--thinking disabled` is a COST control, measured 2026-09-21 against real
 * credits on `qmodel` (identical prompt, CLI print mode):
 *
 *   default              credits 1.103 / 1.009 / 6.765 / 0.579   (erratic)
 *   --thinking disabled  credits 0.637 / 0.546 / 0.546 / 0.552   (stable)
 *   --thinking adaptive  credits 0.739
 *   --reasoning-effort low   credits 0.676
 *   --reasoning-effort high  credits 0.664
 *
 * Two things worth keeping in mind: disabling thinking removes the variance as
 * well as the cost (the default's spread came from unbounded reasoning), and
 * `--reasoning-effort` does NOT reliably save — it measured about the same as
 * adaptive. Set `QODER_CLI_THINKING` to override (e.g. `enabled`, or an empty
 * string to omit the flag entirely).
 *
 * @param {string} model
 * @param {number} contextWindow
 * @param {string} prompt
 * @param {string} [thinkingMode]  `--thinking` value; '' omits the flag.
 * @returns {string[]}
 */
function cliArgs({ model, contextWindow, prompt, thinkingMode = 'disabled' }) {
  return [
    '-p',
...
/** Resolve the `--thinking` mode for a run (env-overridable). */
function resolveThinkingMode(env = process.env) {
  const raw = env?.QODER_CLI_THINKING
  return typeof raw === 'string' ? raw.trim() : 'disabled'
}
...
function runCliPiped({ cliPath, model, contextWindow, prompt, timeoutMs, signal, spawnImpl }) {
  return new Promise((resolve) => {
    ...
    mkdtemp(join(tmpdir(), 'qoder-cli-fallback-'))
      .then((dir) => {
        workDir = dir
        const args = cliArgs({ model, contextWindow, prompt, thinkingMode: resolveThinkingMode() })
        let child
        try {
