...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ACCOUNTS_PATH = '/plugins/dsh-qoder-cli/accounts'
    const ACCOUNTS_ACTIVATE_PATH = '/plugins/dsh-qoder-cli/accounts/activate'
    const ACCOUNTS_PROBE_PATH = '/plugins/dsh-qoder-cli/accounts/probe'
    ...
    // -------------------------------------------------------------------
    // Account-management data layer (t12).
    //
    // Response shapes are taken from the REAL handlers (lib/qoder/web.js),
    // captured over HTTP and recorded in
    // D:\DSH\tmp\p0-crypto\ui-wire\shapes-raw.txt:
    //
    //   GET  /accounts           200  bare array of
    //                                 { uid, email, name, isCurrent, credentialFreshness }
    //                                 uid may be null; freshness is epoch-ms
    //   POST /accounts/activate  200  { success: true, message }
    //                            404  { error }   unknown uid
    //                            400  { error }   uid not a string
    //                            403  { error }   loopback Origin required
    //                            415  { error }   Content-Type must be JSON
    //   POST /accounts/probe     200  { available: boolean, error? }
    //                            404/400/403/415 { error }
    //
    // Every failure path is surfaced as a thrown Error carrying the server's
    // own wording; nothing is swallowed (a silent failure and a success must
    // not look the same).
    // -------------------------------------------------------------------

    /** Turn a non-OK response into an Error that names what actually happened. */
    async function accountError(res, fallbackLabel) {
      let detail = null
      try {
        detail = (await res.json())?.error ?? null
      } catch {
        // non-JSON body: fall back to the status alone
      }
      if (res.status === 403) return new Error(detail ?? '请求未被信任（需要 loopback Origin）')
      if (res.status === 404) return new Error(detail ?? '账号未找到')
      if (res.status === 415) return new Error(detail ?? 'Content-Type 必须是 application/json')
      if (res.status === 405) return new Error(detail ?? '方法不允许')
      if (res.status === 400) return new Error(detail ?? '请求无效')
      return new Error(detail ?? `${fallbackLabel} HTTP ${res.status}`)
    }

    /** List discovered accounts. Throws with a readable message on any failure. */
    async function fetchAccounts() {
      let res
      try {
        res = await fetch(ACCOUNTS_PATH, { headers: { Accept: 'application/json' } })
      } catch (caught) {
        throw new Error(`网络错误：${caught?.message ?? String(caught)}`)
      }
      if (!res.ok) throw await accountError(res, '无法读取账号列表')
      const data = await res.json()
      if (!Array.isArray(data)) {
        throw new Error(`账号列表返回形状异常（期望数组，实得 ${data === null ? 'null' : typeof data}）`)
      }
      return data
    }

    /** Activate one account. Resolves with the server's own result body. */
    async function activateAccount(uid) {
      let res
      try {
        res = await fetch(ACCOUNTS_ACTIVATE_PATH, {
          method: 'POST',
      ...
      if (!res.ok) throw await accountError(res, '激活失败')
      const data = await res.json()
      // The server must say success explicitly; anything else is NOT a success.
      if (data?.success !== true) {
        throw new Error(`服务端未确认成功：${JSON.stringify(data)}`)
      }
      return data
    }

    /** Probe one account's availability. Resolves with `{ available, error? }`. */
    async function probeAccount(uid) {
      let res
      try {
        res = await fetch(ACCOUNTS_PROBE_PATH, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
          body: JSON.stringify({ uid }),
        })
      ...
      if (!res.ok) throw await accountError(res, '探测失败')
      const data = await res.json()
      if (typeof data?.available !== 'boolean') {
        throw new Error(`探测返回形状异常：${JSON.stringify(data)}`)
      }
      return data
    }
    ...
    function QoderPluginCardBody() {
      ...
      // t12: the discovered account list from GET /accounts. `null` = not yet
      // loaded; `[]` = loaded and genuinely empty. Keeping these distinct stops
      // "no data yet" from rendering as "no accounts" (a silent-failure shape).
      const [accounts, setAccounts] = react.useState(null)
      const [accountsError, setAccountsError] = react.useState(null)
      const [accountsBusy, setAccountsBusy] = react.useState(false)
      // t12: per-account action state, keyed by uid. Each entry is
      // { kind: 'activate'|'probe', status: 'running'|'ok'|'failed', text }.
      // A write never renders optimistically: the entry appears as 'running'
      // and only becomes 'ok' when the server says so.
      ...
      const loadAccounts = react.useCallback(async () => {
        ...
        try {
          ...
          // Do NOT keep the previous list: stale rows would read as success.
          setAccounts(null)
          setAccountsError(caught.message)
        } finally {
      ...
      /**
       * The shared ghost/secondary button.
       *
       * Declared HERE, above its first use, because `accountRow` (see below)
       * calls it. It previously sat ~130 lines further down, inside the account
       * actions row. That ordering was invisible while the account list was
       * empty — `accountRow` was never invoked, so the identifier was never
       * evaluated — and threw `ReferenceError: Cannot access 'ghostButton'
       * before initialization` the instant GET /accounts returned a real
       * account. The slot host unmounts an entry whose render throws, which is
       * why the card "appeared, then vanished when clicked".
       */
      ...
      /**
       * Run one account write (activate or probe) and record its REAL outcome.
       *
       * The result entry starts as 'running' so the UI shows progress rather
       * than a premature check mark; it becomes 'ok' only when the server
       * confirms, and 'failed' with the server's own wording otherwise.
       */
        ...
        async (account, kind) => {
          ...
          if (typeof uid !== 'string' || uid.length === 0) {
            setAccountAction((prev) => ({
              ...prev,
              [String(uid)]: { kind, status: 'failed', text: '该账号没有 uid，无法执行该操作' },
            }))
            return
          }
          setAccountAction((prev) => ({ ...prev, [uid]: { kind, status: 'running', text: '进行中…' } }))
          try {
            if (kind === 'activate') {
              const result = await activateAccount(uid)
              setAccountAction((prev) => ({
                ...prev,
                [uid]: { kind, status: 'ok', text: `已激活（${result.message ?? 'ok'}）` },
              }))
              // Re-read both documents: the active account changed server-side.
              void loadAccounts()
              void load()
            } else {
              const result = await probeAccount(uid)
              setAccountAction((prev) => ({
                ...prev,
                [uid]: result.available
                  ? { kind, status: 'ok', text: '探测结果：可用' }
                  : { kind, status: 'failed', text: `探测结果：不可用${result.error ? ` — ${result.error}` : ''}` },
              }))
            }
          } catch (caught) {
            setAccountAction((prev) => ({
              ...prev,
              [uid]: { kind, status: 'failed', text: caught.message },
            }))
          }
        },
        [load, loadAccounts],
                  ...
                  claimableCampaign
                    ? h(
                        'button',
                        {
                          ...
                          title: `${benefitAmount !== null ? `一键签到：领取 ${benefitAmount} credits` : '一键签到：领取活动奖励'}（将在浏览器打开 Qoder 活动页完成领取）`,
                          onClick: (event) => {
                        ...
                        benefitAmount !== null ? `一键签到（${benefitAmount} credits）` : '一键签到',
                      )
                    : claimed
                      ? h(
                          'span',
      ...
      // ------------------------------------------------------------------
      // t12: discovered-account list, driven by GET /accounts.
      //
      // Three distinct states are rendered differently on purpose:
      //   accounts === null && no error  -> still loading
      //   accountsError !== null         -> the failure, in the error colour
      //   accounts === []                -> a genuine empty state
      // Collapsing these is exactly the "failure and silence look alike"
      // defect this workspace keeps a rule about.
      // ------------------------------------------------------------------
      const accountActionNote = (uid) => {
        const entry = accountAction[uid]
        ...
        const colour =
          entry.status === 'failed'
            ...
            : entry.status === 'ok'
              ? '#1a7f37'
              : 'var(--dsw-alias-label-secondary, rgba(128,128,128,0.9))'
        return h(
          'div',
          {
            key: `note-${uid}`,
            style: { fontSize: '12px', lineHeight: 1.5, color: colour, wordBreak: 'break-word', marginTop: '4px' },
          },
          entry.status === 'running' ? entry.text : `${entry.kind === 'activate' ? '激活' : '探测'}：${entry.text}`,
        )
      }

      const accountRow = (account) => {
        const uid = account.uid === null || account.uid === undefined ? null : String(account.uid)
        const name = account.name ?? account.email ?? (uid === null ? '未命名账号' : `${uid.slice(0, 12)}…`)
        const busyKey = uid ?? ''
        const running = accountAction[busyKey]?.status === 'running'
        const chip = account.isCurrent
          ? { text: '当前', style: { background: 'rgba(46,160,67,0.14)', color: '#1a7f37' } }
          : { text: '备选', style: { background: 'rgba(128,128,128,0.14)', color: 'inherit' } }
        const fresh =
          typeof account.credentialFreshness === 'number'
            ? new Date(account.credentialFreshness).toLocaleString('zh-CN', { hour12: false })
            : null
        return h(
          'div',
          {
            key: `acct-${busyKey || 'null'}-${name}`,
            style: {
              border: '0.5px solid var(--dsw-alias-border-l4, rgba(128,128,128,0.4))',
              borderRadius: '8px',
              padding: '8px 10px',
          ...
          h(
            ...
            fresh === null
              ? null
              : h('span', { style: { flex: 'none', fontSize: '11px', opacity: 0.55 } }, fresh),
          ),
          uid === null
            ? h(
                'div',
                { style: { fontSize: '12px', opacity: 0.65, marginTop: '4px' } },
                '该账号未取到 uid（上游 userinfo 不可用），无法激活或探测。',
              )
            : h(
                'div',
                ...
                account.isCurrent
                  ...
                  : ghostButton(
                      running ? '激活中…' : '激活',
                      () => void runAccountAction(account, 'activate'),
                      { fontSize: '12px', padding: '3px 10px', cursor: running ? 'default' : 'pointer' },
                    ),
                ghostButton(
                  running ? '探测中…' : '探测可用性',
                  () => void runAccountAction(account, 'probe'),
                  { fontSize: '12px', padding: '3px 10px', cursor: running ? 'default' : 'pointer' },
                ),
              ),
          accountActionNote(busyKey),
        )
      }

      const accountsPanel = h(
        'div',
        { key: 'accounts-panel', style: { marginBottom: '10px', minWidth: 0 } },
        accountsError !== null
          ? h(
              'div',
              {
              ...
              `无法读取账号列表：${accountsError}`,
            )
          : accounts === null
            ? h('div', { style: { fontSize: '12px', opacity: 0.6 } }, '正在读取账号列表…')
            : accounts.length === 0
              ? h('div', { style: { fontSize: '12px', opacity: 0.6 } }, '未发现任何账号。')
              : h(
                  'div',
                  null,
                  // The active account floats to the top, matching the preview.
                  ...[...accounts]
                    .sort((a, b) => (a.isCurrent === b.isCurrent ? 0 : a.isCurrent ? -1 : 1))
                    .map(accountRow),
                ),
        accountsBusy && accounts !== null
          ? h('div', { style: { fontSize: '11px', opacity: 0.5, marginTop: '2px' } }, '刷新中…')
          : null,
      )

      // ------------------------------------------------------------------
      // Account actions row: sits directly under the Qoder card title and
      // above the tab bar (screenshot layout: "登录账号" + "刷新").
      // ------------------------------------------------------------------
      const loginLabel =
      ...
      const accountActions = h(
        ...
        // Refresh: re-reads the status AND the discovered account list.
        ghostButton('刷新', () => {
          void load()
          void loadAccounts()
        }),
