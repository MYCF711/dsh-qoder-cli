...
async function withReaderShim(fn) {
  ...
  const shim = original
    ...
    // The reader also exports a plain flag; strip its `export` too or the shim
    // has a stray `export` in a non-module position.
    .replace(/\bexport const CREDENTIAL_IS_PURE_JS\b/g, 'const CREDENTIAL_IS_PURE_JS')
...
console.log('')
console.log('== credential failover: the credential cipher primitive ==')
//
// The credential codec was ported to pure JS (AES-128-CBC, key === IV ===
// machine_id.slice(0,16), byte-verified against the former WASM per
// docs/CREDENTIAL-CRYPTO-ANALYSIS.md), and the bundled auth WASM has been
// removed from the package. These tests now exercise the pure implementation
// through the same reader module; the semantics they pin are unchanged.

await test('reader: encrypts and decrypts a credential blob (pure codec)', async () => {
...
await test('reader: the key is the machine id truncated to 16 characters', async () => {
  const reader = await import(pathToFileURL(readerPath).href)
...
await test('FIXED: init() with no argument is safe (the WASM is gone; init is a no-op by default)', async () => {
  const reader = await import(pathToFileURL(readerPath).href)
  // The call shape both production callers use. With the bundled WASM removed,
  // init() resolves without loading any binary; it only wires an optional
  // explicit-path load. The credential codec itself is pure JS and needs no init.
  await reader.init()
})

await test('CLI store: a well-formed store yields the credential through the pure codec', async () => {
  const reader = await import(pathToFileURL(readerPath).href)
  await reader.init()
