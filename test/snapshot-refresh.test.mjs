...
test('negative: catalog with only empty scenes → exit 1 "no non-empty scene", no output file', async () => {
  ...
  // gets a real (non-null) catalog document and must still refuse. The fixture
  // is built with the pure JS codec (the bundled WASM has been removed).
  ...
  const encrypted = pure.modelCacheEncrypt(JSON.stringify({ assistant: [], chat: [], app: [], qwake: [] }), uid)
