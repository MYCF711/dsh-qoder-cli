...
export function apply(ctx, config) {
   ...
   */
  const configHome = process.env.QODER_CONFIG_DIR ?? null
      ...
      if (!gateCache && !gateProbing && entries.length > 0) {
        ...
        void (async () => {
          try {
            ...
            const refined = await buildCatalog({ dshHome, gateCache: { measuredAt: new Date().toISOString(), results } })
  ...
  ctx.inject(['webServer'], (webCtx) => {
    registerQoderRoutes(webCtx, {
      ...
      catalogDump: async () => {
        const gateCache = dshHome ? await loadGateCache(dshHome) : null
        return buildCatalog({ dshHome, gateCache })
      },
      deviceLogin,
