...
import { cliFallbackSse } from './cli-fallback.js'
...
export function createQoderShim(options) {
  ...
  async function chatCompletions(req, res) {
    ...
    if (!result.ok) {
      ...
      // CLI subprocess fallback (qfmodel etc.): the REST gateway rejects some
      // models the local CLI serves through its private chat path. When the
      // failure shape matches, run the CLI once and answer with a complete SSE
      // body. A refusal here must not mask the original upstream error.
      const fallback = await cliFallbackSse({ body, failure: result, signal: controller.signal })
      if (fallback.ok) {
        res.writeHead(200, {
          'Content-Type': 'text/event-stream',
          'Cache-Control': 'no-cache',
