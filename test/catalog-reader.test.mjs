...
try {
  ...
  modelCacheDecrypt = wasmHelper.modelCacheDecrypt
...
if (WASM_PATH && modelCacheEncrypt && decryptCatalog) {
  // ------------------------------
  // FIXTURES
  // ------------------------------
  try {
...
if (fixtureChat && typeof fixtureChat.chat === 'object') {
  ...
  // ------------------------------
  await testAsync('rawModelToCatalogEntry: is_vl=true yields input includes image', async () => {
    ...
    assert.ok(entry.input.includes('image'), 'is_vl=true must include image in input')
